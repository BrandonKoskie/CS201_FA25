...
-   Type your answer(s) in bullet points below
    ...
    -   dlookr - Gives a more comprehensive report, including missing values, distribution, and correlation.

    -   DataExplorer - Makes a full EDA report, including plots.

    -   Hmisc - Gives class, missing values, percentiles, and counts for categorical variables.

    -   psych - Useful for numeric summaries and descriptive statistics. Includes mean, sd, median, min, max, range, skew, and kurtosis.

    -   summarytools - Generates pretty HTML/text tables for descriptive stats.
...
```{r, message=FALSE}
penguin_flipper_plot <- ggplot(data = penguins_data, aes(x = species, y = flipper_length_mm, fill = species)) +
  geom_boxplot(outlier.shape = NA) +    points with jitter
  geom_jitter(size = 0.5, width = 0.2, alpha = 0.7) + 
  theme_bw() +
  scale_fill_viridis_d(option = "plasma") +
  labs(
    title = "Palmer Penguins: Distribution of Flipper Length",
    subtitle = "Flipper length for 3 penguin species on Palmer Archipelago",
    x = "Species",
    y = "Flipper Length (mm)",
    fill = "Species"
  ) +
