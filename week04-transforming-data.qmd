...
```{r, message=FALSE}
penguin_scatter_plot <- ggplot(data = penguins_data, 
...
```{r, message=FALSE}
ggplot(average_flipper_length, aes(x = species, y = avg_flipper, fill = species)) +
...
```{r, message=FALSE}
body_mass_boxplot <- ggplot(penguins_data, aes(x = species, y = body_mass_g, fill = species)) +
  ...
  labs(
    ...
    subtitle = "Data from Palmer Archipelago, Antartica",
    x = "Species",
    y = "Body Mass(g)",
    fill = "Species"
  )

ggplotly(body_mass_boxplot)
```
...
```{r, message=FALSE}
avg_culmen_depth <- penguins_data %>%
  group_by(species) %>%
  summarise(avg_depth = mean(culmen_depth_mm, na.rm = TRUE))


avg_culmen_depth

ggplot(avg_culmen_depth, aes(x = species, y = avg_depth, fill = species)) +
  geom_bar(stat = "identity") +
  labs(
    title = "Average Culmen Depth by Penguin Species",
    x = "Species",
