...
author: "Amber Camp (edited by Brandon Koskie)" 
date: "2025-09-08"
format: html
editor: visual
---
...
-   This is a float point error in R where the numbers are stored in binary which causes a small rounding error. So instead of 0.1 it would be 0.9999999 etc.
...
-   How might you view the last few rows of the dataframe with a single function?
    ...
    tail(your_dataframe) so tail(mtcars)
...
Create a scatterplot and histogram and box plot of two variables of your choice (but not `mpg` vs. `wt`). Feel free to try a `ggplot2` version if you have some R experience!

```{r}
plot(mtcars$cyl, mtcars$mpg,
     xlab = "Cylinders", ylab = "MPG",
     main = "Cars with more cylinders tend to have lower MPG")
```
...
hist(mtcars$wt, main = "Weight (wt)", col = "tomato", xlab = "Weight")
hist(mtcars$mpg, main = "Miles per Gallon (mpg)", col = "skyblue", xlab = "MPG")
