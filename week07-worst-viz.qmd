...
ggplot(mtcars, aes(x = mpg, y = hp, size = wt, color = cyl, label = car)) +
  geom_line(aes(group = 1), color = "magenta", linewidth = 3, linetype = "dotted") +
  geom_point(shape = 23, fill = "chartreuse", color = "cyan", alpha = 0.5, stroke = 3) +
  geom_bar(stat = "identity", aes(fill = factor(gear)), width = 2, alpha = 0.7) +
  geom_text(angle = 75, size = 3, color = "red", fontface = "bold") +
  labs(
    title = "Car go BRR?",
    subtitle = "to be car or not to be",
    x = "Time (probably)",
    y = "Efficiency (maybe)",
  ...
  theme_dark() +
  theme(
    plot.background = element_rect(fill = "brown"),   # fixed color
    panel.background = element_rect(fill = "deeppink"),
    panel.grid.major = element_line(color = "orange", linewidth = 2),
    panel.grid.minor = element_line(color = "yellow", linetype = "dashed"),
    axis.text.x = element_text(angle = 120, size = 14, color = "cyan"),
    axis.text.y = element_text(size = 12, color = "red", face = "italic"),
    plot.title = element_text(family = "Comic Sans MS", size = 26, face = "bold", color = "hotpink"),
    plot.subtitle = element_text(family = "Papyrus", size = 18, color = "gold"),
    legend.background = element_rect(fill = "aqua"),
    legend.key = element_rect(fill = "purple"),
    legend.text = element_text(size = 10, family = "Zapfino", color = "chartreuse")
  ) +
  coord_polar()
...
# --- make up some data ---
bar_chaos <- tibble(
  car = c("CarA","CarB","CarC","CarD","CarE"),
  mpg = c(10,15,25,30,18),
  cyl = c(4,6,8,4,6),
  image = c(
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSe_SiUf5YtDPcfFVjuaHO8CVUaON7qTW1KGA&s",  # emoji face
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSe_SiUf5YtDPcfFVjuaHO8CVUaON7qTW1KGA&s",  # cat face
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSe_SiUf5YtDPcfFVjuaHO8CVUaON7qTW1KGA&s",  # frog face
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSe_SiUf5YtDPcfFVjuaHO8CVUaON7qTW1KGA&s",
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSe_SiUf5YtDPcfFVjuaHO8CVUaON7qTW1KGA&s"
  )
)

# --- plot: intentionally ugly ---
ggplot(bar_chaos, aes(x = car, y = mpg, fill = factor(cyl))) +
  geom_col(alpha = 0.4, color = "hotpink") +
  geom_image(
    aes(image = image, angle = mpg * 10),
    size = 0.18,
    by = "height",
    asp = 1
  ) +
  labs(
    title = "Car go BRR",
    x = "Vehicular Entities (???)",
    y = "Some Measurement (who knows)",
    caption = "Source: the void"
  ) +
  theme(
    plot.background = element_rect(fill = "limegreen"),
    panel.background = element_rect(fill = "deeppink"),
    axis.text.x = element_text(angle = 85, size = 12, color = "yellow"),
    axis.text.y = element_text(size = 12, color = "red", face = "italic"),
    plot.title = element_text(family = "Comic Sans MS", color = "cyan", size = 24)
  ) +
  coord_polar()
```
